...
const population = function(pop) {
    if (pop.length === 4 || pop.length === 5 || pop.length === 6) return 'thousand';
...
const renderCountry = function(res) {
  ...
  <div class="country__data">
    ...
    <p class="country__row"><span>👫</span>${country.population} ${population(String(country.population))} people</p>
...
const whereAmILocated = async function() {
    ...
    // Reverse geo-coding
    const resCityJSON = await getJSON(`https://geocode.xyz/${latitude},${longitude}?geoit=json&auth=105550322406220270442x3850`);
    // Render city
    renderCity(resCityJSON);
    // Get country based on city object
    const { country  } = resCityJSON;
    const resCountryJSON = await getJSON(`https://restcountries.com/v2/name/${country}`);
    // Render country
